...
- m_demo_mapping2 compares AES_DECRYPT(lookup-column, 3-char key, 256) with
  MD5(source-columns); the value spaces are incomparable, so every matched
  lookup row is flagged Update.
...
- agg_TRANS groups by ACCT_ID as marked by EXPRESSIONTYPE=GROUPBY. TX_AMT is a
  per-account total and all other pass-through ports use the last row in each
  account's sorted SB input. Determinism is highest TX_ID within each account.
"""
import argparse
import os
...
def load(con, trigger_abort=False):
    def path(f):
    ...
    con.execute(f"""
        CREATE OR REPLACE TABLE demo_target1_pre_raw AS
        SELECT row_number() OVER () AS __seed_row, *
        FROM read_csv('{path('demo_target1.csv')}', header=true, columns={{
    ...
    con.execute("""
        CREATE OR REPLACE TEMP VIEW demo_target1_pre AS
        SELECT * EXCLUDE (__seed_row) FROM (
            SELECT *, ROW_NUMBER() OVER (
                PARTITION BY ID ORDER BY "Key" DESC, __seed_row DESC
            ) AS rn
            FROM demo_target1_pre_raw
        ) WHERE rn = 1""")
    con.execute(f"""
    ...
    con.execute(f"""
        CREATE OR REPLACE TABLE lkp_demo_source1_raw AS
        SELECT row_number() OVER () AS __seed_row, *
        FROM read_csv('{path('lkp_demo_source1.csv')}', header=true, columns={{
            ...
            'CUST_TYP':'VARCHAR'}})""")
    con.execute("""
        CREATE OR REPLACE TEMP VIEW lkp_demo_source1 AS
        SELECT * EXCLUDE (__seed_row) FROM (
            SELECT *, ROW_NUMBER() OVER (PARTITION BY ACCT_ID ORDER BY __seed_row DESC) AS rn
            FROM lkp_demo_source1_raw
        ) WHERE rn = 1""")
    con.execute(f"""
        CREATE OR REPLACE TABLE lkp_demo_source2_raw AS
        SELECT row_number() OVER () AS __seed_row, *
        FROM read_csv('{path('lkp_demo_source2.csv')}', header=true, columns={{
    ...
    con.execute("""
        CREATE OR REPLACE TEMP VIEW lkp_demo_source2 AS
        SELECT * EXCLUDE (__seed_row) FROM (
            SELECT *, ROW_NUMBER() OVER (PARTITION BY CUST_ID ORDER BY __seed_row DESC) AS rn
            FROM lkp_demo_source2_raw
        ) WHERE rn = 1""")
    con.execute(f"""
        CREATE OR REPLACE TABLE lkp_demo_source3_raw AS
        SELECT row_number() OVER () AS __seed_row, *
        FROM read_csv('{path('lkp_demo_source3.csv')}', header=true, columns={{
            'ACCT_ID':'BIGINT','TX_TYPE_CD':'VARCHAR','TX_TYPE_DESC':'VARCHAR'}})""")
    con.execute("""
        CREATE OR REPLACE TEMP VIEW lkp_demo_source3 AS
        SELECT * EXCLUDE (__seed_row) FROM (
            SELECT *, ROW_NUMBER() OVER (PARTITION BY ACCT_ID ORDER BY __seed_row DESC) AS rn
            FROM lkp_demo_source3_raw
        ) WHERE rn = 1""")
    con.execute(f"""
    ...
    source2_name = 'abort/demo_source2.csv' if trigger_abort else 'demo_source2.csv'
    con.execute(f"""
        CREATE OR REPLACE TABLE demo_source2 AS
        SELECT * FROM read_csv('{path(source2_name)}', header=true, columns={{
...
def mapping1(con):
    ...
    t6 = con.execute("""
        WITH sb AS (
            SELECT *,
                   ROW_NUMBER() OVER (
                       PARTITION BY ACCT_ID ORDER BY TX_ID DESC
                   ) AS rn,
...
def mapping2(con):
    con.execute("""
        ...
        SELECT s.*,
               ...
               -- The legacy compares AES_DECRYPT(lookup-column, key, 256)
               -- with MD5(source-columns): incomparable value spaces.
               ...
               CASE WHEN p."Key" IS NOT NULL THEN 'Update' END AS Changed_Flag,
        ...
        LEFT JOIN (
            SELECT * FROM (
                SELECT p.*, ROW_NUMBER() OVER (PARTITION BY ID ORDER BY "Key" DESC) AS rn
                FROM demo_target1_pre p
            ) ranked WHERE rn = 1
        ) p ON p.ID = s.ID""")
...
def main():
    ...
    parser.add_argument("--trigger-abort", action="store_true",
                        help="load the optional abort seed row set")
    args = parser.parse_args()
    con = duckdb.connect()
    load(con, trigger_abort=args.trigger_abort)
    mapping1(con)
    mapping2(con)
    mapping3(con)
