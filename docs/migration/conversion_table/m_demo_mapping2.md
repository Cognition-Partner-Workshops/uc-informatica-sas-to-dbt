...
| m_demo_mapping2 | UPDTRANS | Forward Rejected Rows | `YES` | 342 | `m_demo_mapping2.py:65` (rows are retained in the update instance) | MEDIUM | No rejected rows occur in the seed; a wrong rejection treatment would only appear on rejected input and parity would not catch it here. |
| m_demo_mapping2 | Mapping instances | `demo_target1_INS`, `demo_target1_UPD` | `INSTANCE` elements over target definition `demo_target1` | 345-346 | `m_demo_mapping2.py:10` and returned dict at `m_demo_mapping2.py:104-107` | LOW (DECISION-6) | Rejected unioning instances into one output; separate materialisation preserves the INS/UPD parity control. |
...
| m_demo_mapping2 | RTRTRANS | LEAD_CO_MNE3; BRANCH_CO_MNE3; MIS_DATE3; ID2; DESCRIPTION3; SHORT_NAME3; LEAD_CO_MNE12; BRANCH_CO_MNE12; MIS_DATE12; DESCRIPTION12; SHORT_NAME12; New_Flag2; Changed_Flag2; o_CREATED_BY2; o_CREATED_TIME2; o_UPDATED_BY2; o_UPDATED_TIME2; Key2 | `REF_FIELD` ports in DEFAULT1 group | 245-262 | Not converted | NOT MIGRATED | These DEFAULT1 ports are unconnected. Their names collide exactly with UPDTRANS ports, but matching by name would incorrectly treat discarded DEFAULT1 rows as updates. |
| m_demo_mapping2 | UPDTRANS | LEAD_CO_MNE3; BRANCH_CO_MNE3; MIS_DATE3; ID2; DESCRIPTION3; SHORT_NAME3; o_UPDATED_BY2; o_UPDATED_TIME2; Key2 | `INPUT/OUTPUT` ports connected to demo_target1_UPD | 328-333, 338-340 | `m_demo_mapping2.py:90-102` and return at `m_demo_mapping2.py:104-107` | HIGH | These are migrated UPDTRANS ports. They intentionally share names with the unconnected RTRTRANS DEFAULT1 ports; their CONNECTOR edges carry Update-group rows to the target. |
| m_demo_mapping2 | UPDTRANS | New_Flag2; Changed_Flag2; o_CREATED_BY2; o_CREATED_TIME2 | `INPUT/OUTPUT` ports with no outgoing connector | 334-337 | Not converted | NOT MIGRATED | These UPDTRANS ports are genuinely unconnected; unlike the same-named routed fields, they do not reach either target. |
...
Counts: HIGH 20, MEDIUM 8, LOW 4, NOT MIGRATED 6 (38 rows total). LOW rows rest on DECISION-3 (lookup policy), DECISION-4 (AES sentinel), DECISION-5 (sequence order), and DECISION-6 (per-instance materialisation).
