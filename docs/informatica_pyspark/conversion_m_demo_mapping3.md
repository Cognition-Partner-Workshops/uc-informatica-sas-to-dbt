...
- Total comparison rows: **22**
- Expression rows: **15**
- Source Qualifier attribute rows: **2**
- Router group rows: **4**
- Migrated rows: **18**
- Not migrated by design: **4**
- Confidence: **HIGH 16, MEDIUM 2, LOW 0, NOT MIGRATED 4**
...
semantic choice. The physical row order is not used by this mapping. The
MEDIUM rows are the numeric rendering cast (not distinguishable by
`pandas.read_csv` parity comparison) and the sparsely populated suffix port.
...
4. **Control blind spots:** the five numeric casts and empty-vs-NULL suffix
   behavior are intentionally documented as MEDIUM because the baseline/parity
   control cannot distinguish those cases.
